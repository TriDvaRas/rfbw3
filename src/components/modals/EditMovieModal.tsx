...
import { MovieDLCSchema, MovieFormSchema, movieFormSchema } from './CreateMovieModal';
...
interface Props {
  ...
  defaultValues: MovieFormSchema
}

const EditMovieModal: React.FC<Props> = (props) => {
  const [showValidationErrors, setShowValidationErrors] = useState(false)
  const { data: me } = api.players.getMyPlayer.useQuery()
  const [collapsedDlcs, setCollapsedDlcs] = useState<number[]>([])

  const { mutate: updateMovieContentMutation, isLoading: isSaving } = api.content.updateMovieContent.useMutation({
    onSuccess() {
