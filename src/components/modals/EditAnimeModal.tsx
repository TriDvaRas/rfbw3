...
import { AnimeDLCSchema, AnimeFormSchema, animeFormSchema } from './CreateAnimeModal';
...
interface Props {
  ...
  defaultValues: AnimeFormSchema
}

const EditAnimeModal: React.FC<Props> = (props) => {
  const [showValidationErrors, setShowValidationErrors] = useState(false)
  const { data: me } = api.players.getMyPlayer.useQuery()
  const [collapsedDlcs, setCollapsedDlcs] = useState<number[]>([])

  const { mutate: updateAnimeContentMutation, isLoading: isSaving } = api.content.updateAnimeContent.useMutation({
    onSuccess() {
      ...
      <Modal open={props.open} onClickBackdrop={() => props.closeOnBackdropClick && props.onClose()} className="w-11/12 max-w-7xl ">
        ...
        <Modal.Body className="flex flex-row w-full max-h-full">
          ...
          <div className="w-1/3 max-h-full">
            <Modal.Header className="mb-2">Редактирование Аниме</Modal.Header>
