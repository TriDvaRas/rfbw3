...
function LazyImage({ height, src, width, className, alt,imageClassName }: Props) {
    ...
    return <div style={customImgDimension} className={`flex ${className} `}>
        ...
        {
            imageLoadedState == 1 && <NextImage quality={100} src={src} alt={''} width={_width} height={_height} className={`min-w-full min-h-full object-cover blur ${imageClassName}`}
                onLoadingComplete={(e) => {
                    setImageLoadedState(2)
                }} />
        }
        {
            imageLoadedState == 2 && <NextImage quality={100} src={src} alt={''} width={_width} height={_height} className={`min-w-full min-h-full object-cover ${imageClassName}`} />
        }
