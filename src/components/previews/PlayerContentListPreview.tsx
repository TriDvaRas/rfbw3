...
function PlayerContentListPreview({ content }: Props) {
    ...
    return (
        ...
        >
            {
                content.map((content, i) => <ContentPreview key={i} approved={content.isApproved} className="scale-75 -my-8 m" label={content.label} type={content.type} imageUrl={content.imageId} />)
            }
        </div>
    )
}
